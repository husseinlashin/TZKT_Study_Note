...
class BtDownload():
    ...
    def __init__(self):
        ...
        # self.url = "https://www.51cc.co/list?q=passion-hd%201080p.MP4-KTR%5Brarbg%5D&page={}"
        self.url = "https://www.51cc.co/list?q=babes%201080p.MP4-KTR%5Brarbg%5D&page={}"
        # 使用代理IP
    ...
    def get_bt(self, html):  # 解析数据,获取一页的bt信息
        ...
        for bt in bts:
            ...
            # 加入去重，如果magnet值已经存在与列表的字典中，则不加入，跳过
    ...
    def save_bt(self, bt_list):
        with open("babes.txt", "a", encoding="utf-8") as f:
            for bt in bt_list:
                print(bt["magnet"])
                f.write(bt["magnet"] + "\n")
