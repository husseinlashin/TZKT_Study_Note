...
def get_date(url_list):
    ...
    for url in url_list:
        ...
        with open("tushy_date.txt", "a", encoding="utf-8") as f:
            for date in date_list:
                # July 18, 2019修改为Vixen.19.07.18
                # 先替换, 20为, Vixen.
                date = date.replace(", 20", ", Tushy.")
                # 匹配分组，交换日期的顺序,中间用.连接
                date_new = re.sub(r'([a-zA-Z]+) (\d+), (Tushy.\d+)', r'\3.\1.\2', date)
                # 分别进行月份替换，月份替换成数字月份
                ...
                f.write(date_new + "\n")
            # 最原始的日期也保存一份
            with open("tushy_date_list.txt", "a", encoding="utf-8") as f:
                f.write(url + "：爬取完成" + "\n")
...
if __name__ == '__main__':
    url_list = ["https://www.tushy.com/videos?page=" + str(i) + "&size=12" for i in range(1, 27)]
    get_date(url_list)
